...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      document.cookie = `scrumflow_token=${data.access_token}; path=/; max-age=${60 * 60 * 24 * 7}`;

      router.push(homeRouteForRole(data.system_role));
    } catch (err: any) {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="mb-8 text-center">
          <div className="inline-flex items-center gap-2 mb-2">
            ...
            <span className="text-white font-semibold text-lg tracking-tight">ScrumFlow</span>
          </div>
          <p className="text-[#5a5a66] text-sm">Sign in to your workspace</p>
        </div>
        ...
        <div className="bg-[#111114] border border-[#1e1e24] rounded-xl p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm text-[#8a8a99] mb-1.5">Email</label>
              <input
                type="email"
                value={email}
            ...
            <div>
              <label className="block text-sm text-[#8a8a99] mb-1.5">Password</label>
              <input
                type="password"
                value={password}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#7c6aff] hover:bg-[#6b59ee] disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium text-sm rounded-lg py-2.5 transition-colors"
            >
