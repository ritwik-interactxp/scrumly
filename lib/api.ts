...
export const membersApi = {
  ...
  invite: (projectId: string, email: string, name: string, role: string) =>
    api
      .post(`/projects/${projectId}/invite`, { email, name, role_in_project: role })
      .then((r) => r.data),
...
export const modulesApi = {
  ...
  update: (
    projectId: string,
    moduleId: string,
    data: Partial<{ title: string; description: string; status: string; doc_link: string }>
  ) =>
...
export const checklistApi = {
  list: (projectId: string, moduleId: string) =>
    api
      .get<ChecklistItem[]>(`/projects/${projectId}/modules/${moduleId}/checklist`)
      .then((r) => r.data),

  add: (projectId: string, moduleId: string, text: string) =>
    api
      .post<ChecklistItem>(`/projects/${projectId}/modules/${moduleId}/checklist`, { text })
      .then((r) => r.data),

  toggle: (projectId: string, itemId: string) =>
    api
      .patch<ChecklistItem>(`/projects/${projectId}/checklist/${itemId}/toggle`)
      .then((r) => r.data),

  update: (projectId: string, itemId: string, text: string) =>
    api
      .patch<ChecklistItem>(`/projects/${projectId}/checklist/${itemId}`, { text })
      .then((r) => r.data),
