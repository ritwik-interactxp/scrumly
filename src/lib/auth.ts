...
export function saveAuth(data: AuthToken) {
  ...
  localStorage.setItem(USER_KEY, JSON.stringify({ id: data.user_id, name: data.name, system_role: data.system_role }));
}
...
export function getRole(): SystemRole | null { return getUser()?.system_role ?? null; }
...
export function homeRouteForRole(role: SystemRole): string {
  switch (role) {
    ...
    case "colleague": return "/workspace";
