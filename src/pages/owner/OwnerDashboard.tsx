        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={s.label} className="bg-[#111114] border border-[#1e1e24] rounded-xl p-4">
              ...
              <p className="text-2xl font-mono font-semibold">{s.value}</p>
            </div>
          ))}
        </div>

        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-medium text-[#8a8a99] uppercase tracking-wider">Projects</h2>
              ...
              <Link key={project.id} to={`/owner/dashboard/${project.id}`}
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ background: project.status === "active" ? "#22c55e" : "#5a5a66" }} />
                    <span className="font-medium">{project.name}</span>
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-full border ${project.status === "active" ? "border-green-500/20 bg-green-500/10 text-green-400" : "border-[#2e2e38] bg-[#1a1a20] text-[#5a5a66]"}`}>
                    {project.status}
                  </span>
                </div>
                {project.description && <p className="text-sm text-[#5a5a66] mb-3 line-clamp-1">{project.description}</p>}
          ...
          <div className="bg-[#111114] border border-[#1e1e24] rounded-xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => setShowNew(false)}
                className="flex-1 border border-[#1e1e24] text-[#8a8a99] hover:text-white text-sm font-medium py-2.5 rounded-lg transition-colors">Cancel</button>
