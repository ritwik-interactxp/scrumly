    ...
    <div className="min-h-screen bg-[#0d0d0f] text-white flex flex-col">
                    ...
                    return (
                      <div
                        ...
                        className={`bg-[#111114] border rounded-xl p-4 cursor-pointer transition-all group ${
                          isSelected
                            ? "border-[#7c6aff] shadow-[0_0_0_1px_#7c6aff22]"
        ...
        {selectedModule && activeTab === "modules" && (
          <div className="w-80 border-l border-[#1e1e24] flex flex-col bg-[#0d0d0f] flex-shrink-0">
            <div className="p-4 border-b border-[#1e1e24] flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-xs text-[#5a5a66] mt-0.5">
                  {selectedModule.checklist_done}/{selectedModule.checklist_total} tasks · {selectedModule.progress}%
                </p>
              </div>
              <button
                onClick={() => setSelectedModule(null)}
                className="text-[#5a5a66] hover:text-white text-xl leading-none flex-shrink-0 mt-0.5"
              >
                ×
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-1">
              {checklist.length === 0 && (
                <p className="text-xs text-[#5a5a66] text-center py-8">No tasks yet. Add one below.</p>
              )}
              {checklist.map((item) => (
                <div
                  key={item.id}
                  className="flex items-start gap-2.5 py-1.5 px-2 rounded-lg hover:bg-[#111114] cursor-pointer group transition-colors"
                ...
                >
                  <div
                    className={`mt-0.5 w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center transition-all ${
                      item.is_done
                        ...
                        : "border-[#2e2e38] group-hover:border-[#7c6aff]"
                    }`}
                  >
                    {item.is_done && (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm leading-snug ${item.is_done ? "text-[#3a3a44] line-through" : "text-white"}`}>
                      {item.text}
                    </p>
                    {item.is_done && item.done_by_name && (
                      <p className="text-xs text-[#2e2e38] mt-0.5">by {item.done_by_name}</p>
                    )}
                  </div>
                </div>
              ))}
        ...
        <Modal onClose={() => { setShowInvite(false); setInviteLink(""); setInviteEmail(""); setInviteName(""); }} title="Invite to Project">
          {inviteLink ? (
            <div className="space-y-4">
              ...
              <div className="bg-[#0d0d0f] border border-[#1e1e24] rounded-lg px-3 py-3 text-xs text-[#7c6aff] break-all leading-relaxed">
                {inviteLink}
              </div>
              <button
                onClick={copyInviteLink}
                className={`w-full ${copySuccess ? "bg-emerald-500 hover:bg-emerald-600" : "bg-[#7c6aff] hover:bg-[#6b59ee]"} text-white text-sm py-2.5 rounded-lg transition-colors`}
              >
                {copySuccess ? "✓ Copied!" : "Copy Invite Link"}
              </button>
              <button
                onClick={() => { setShowInvite(false); setInviteLink(""); setInviteEmail(""); setInviteName(""); }}
                className="w-full text-[#5a5a66] text-sm py-2 hover:text-white transition-colors"
              >
                Done
              </button>
            </div>
            ...
            <>
              <div className="space-y-3">
                ...
                <div>
                      ...
                      <button
                        ...
                        className={`flex-1 py-2.5 text-sm rounded-lg border transition-colors capitalize ${
                          inviteRole === r
                            ...
                            : "border-[#1e1e24] text-[#5a5a66] hover:text-white hover:border-[#2e2e38]"
                        }`}
                      >
                        {r}
                      </button>
                  ...
                  <p className="text-xs text-[#3a3a44] mt-2">
                    {inviteRole === "client"
                      ? "Clients see a simplified read-only portal view."
                      : "Colleagues can interact with tasks and modules."}
                  </p>
                </div>
              </div>
              <div className="flex gap-3 mt-5">
      ...
      {confirmDeleteModule && (
        <Modal onClose={() => setConfirmDeleteModule(null)} title="Delete Module">
          <p className="text-sm text-[#8a8a99] mb-2">
            Are you sure you want to delete{" "}
            <span className="text-white font-medium">"{confirmDeleteModule.title}"</span>?
          </p>
          <p className="text-xs text-[#5a5a66] mb-5">
            This will permanently remove the module and all its tasks. This cannot be undone.
          </p>
          <div className="flex gap-3">
            <button onClick={() => setConfirmDeleteModule(null)} className={BTN_GHOST}>Cancel</button>
            <button
              onClick={() => deleteModule(confirmDeleteModule)}
    ...
    <div className="bg-[#111114] border border-[#1e1e24] rounded-xl px-4 py-3 flex items-center justify-between gap-3 group hover:border-[#2e2e38] transition-colors">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        {member.status === "pending" && (
          <span className="text-xs text-[#5a5a66] border border-[#2e2e38] rounded-full px-2 py-0.5">
            pending
          </span>
        )}
        <button
          onClick={onRemove}
          className="opacity-0 group-hover:opacity-100 text-[#3a3a44] hover:text-red-400 transition-all text-sm px-2 py-1 rounded"
...
function Modal({ children, onClose, title }: { children: React.ReactNode; onClose: () => void; title: string }) {
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div
        className="bg-[#111114] border border-[#1e1e24] rounded-xl w-full max-w-md p-6 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
...
const INPUT =
  "w-full bg-[#0d0d0f] border border-[#1e1e24] rounded-lg px-3 py-2.5 text-white text-sm placeholder:text-[#3a3a44] focus:outline-none focus:border-[#7c6aff] transition-colors";
const BTN_PRIMARY =
  "flex-1 bg-[#7c6aff] hover:bg-[#6b59ee] disabled:opacity-50 text-white text-sm py-2.5 rounded-lg transition-colors";
const BTN_GHOST =
  "flex-1 border border-[#1e1e24] text-[#8a8a99] hover:text-white text-sm py-2.5 rounded-lg transition-colors";
