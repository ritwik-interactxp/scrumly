...
export default function PortalHome() {
  ...
  useEffect(() => {
    projectsApi.list().then((list) => {
      setProjects(list);
      if (list.length === 1) navigate(`/portal/${list[0].id}`, { replace: true });
    ...
    <div className="min-h-screen bg-[#0d0d0f] text-white">
      <header className="border-b border-[#1e1e24] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold">Scrumly</span>
        </div>
        <button onClick={logout} className="text-xs text-[#5a5a66] hover:text-white transition-colors">Sign out</button>
      </header>
          ...
          {projects.map((p) => (
            <Link key={p.id} to={`/portal/${p.id}`}
              ...
              <div>
                ...
                {p.description && <p className="text-sm text-[#5a5a66] mt-0.5">{p.description}</p>}
              </div>
              <div className="flex items-center gap-3">
