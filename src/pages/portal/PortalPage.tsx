...
function CircleProgress({ value, size = 64 }: { value: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} className="-rotate-90">
      ...
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={value === 100 ? "#22c55e" : "#7c6aff"}
  ...
  return (
    <div className="bg-[#111114] border border-[#1e1e24] rounded-2xl p-6">
      <div className="flex items-start justify-between gap-4 mb-4">
        ...
        <span className="text-xs px-2.5 py-1 rounded-full whitespace-nowrap flex-shrink-0" style={{ color: sc.color, background: sc.bg }}>{sc.label}</span>
      </div>
      <div className="flex items-center gap-4">
        <div className="relative">
          <CircleProgress value={mod.progress} size={56} />
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-xs font-mono font-semibold" style={{ color: mod.progress === 100 ? "#22c55e" : "#7c6aff" }}>{Math.round(mod.progress)}%</span>
          </div>
        </div>
        <div className="flex-1">
          <div className="h-1.5 bg-[#1e1e24] rounded-full overflow-hidden mb-2">
            <div className="h-full rounded-full transition-all duration-700" style={{ width: `${mod.progress}%`, background: mod.progress === 100 ? "#22c55e" : "#7c6aff" }} />
          </div>
      ...
      {mod.doc_link && (
        <div className="mt-4 pt-4 border-t border-[#1e1e24]">
          <a href={mod.doc_link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm text-[#7c6aff] hover:text-white transition-colors">
...
export default function PortalPage() {
  ...
  useEffect(() => {
    if (!projectId) return;
    portalApi.get(projectId).then(setData).catch(() => setError("Unable to load project. Please check your access.")).finally(() => setLoading(false));
  }, [projectId]);

  if (loading) return <div className="min-h-screen bg-[#0d0d0f] flex items-center justify-center"><p className="text-[#5a5a66]">Loading...</p></div>;
  if (error || !data) return (
    <div className="min-h-screen bg-[#0d0d0f] flex items-center justify-center p-4">
      <div className="text-center"><p className="text-red-400 mb-2">{error}</p><a href="/auth/login" className="text-sm text-[#7c6aff] hover:underline">Sign in</a></div>
    </div>
  );
        ...
        <div className="mb-10">
          ...
          <div className="mt-6 bg-[#111114] border border-[#1e1e24] rounded-2xl p-6">
            ...
            <div className="h-2 bg-[#1e1e24] rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-700" style={{ width: `${data.overall_progress}%`, background: data.overall_progress === 100 ? "#22c55e" : "#7c6aff" }} />
            </div>
            <div className="flex gap-6 mt-4 text-sm text-[#5a5a66]">
              <span>{data.modules.length} modules total</span><span>{doneModules} completed</span>
            </div>
          </div>
        </div>
        <h2 className="text-sm font-medium text-[#5a5a66] uppercase tracking-wider mb-4">Modules</h2>
