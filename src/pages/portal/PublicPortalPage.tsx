        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "16px 24px 20px", animation: "expandDown 0.25s ease" }}>
          {mod.checklist_total === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              ...
              {(mod.checklist_items && mod.checklist_items.length > 0
                ? mod.checklist_items
                : Array.from({ length: mod.checklist_total }).map((_, i) => ({
                    text: `Task ${i + 1}`,
                    is_done: i < mod.checklist_done,
                  }))
              ).map((item, i) => (
                <div key={i} style={{
                  display: "flex", alignItems: "center", gap: "10px",
                  padding: "8px 12px", borderRadius: "10px",
                  background: item.is_done ? "rgba(34,197,94,0.06)" : "rgba(255,255,255,0.03)",
                  border: `1px solid ${item.is_done ? "rgba(34,197,94,0.15)" : "rgba(255,255,255,0.05)"}`,
                }}>
                  <div style={{
                    width: "16px", height: "16px", borderRadius: "50%", flexShrink: 0,
                    background: item.is_done ? "#22c55e" : "transparent",
                    border: `2px solid ${item.is_done ? "#22c55e" : "#334155"}`,
                  ...
                  <span style={{ fontSize: "13px", color: item.is_done ? "#64748b" : "#94a3b8", textDecoration: item.is_done ? "line-through" : "none", flex: 1 }}>
                    {item.text}
                  </span>
  ...
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()}
      ...
      <div style={{
        ...
        border: "1px solid rgba(124,106,255,0.3)", borderRadius: "24px", padding: "28px",
          ...
          {loading && (
            <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "20px 0" }}>
              <div style={{ display: "flex", gap: "4px" }}>
                {[0, 1, 2].map((i) => (
                  <div key={i} style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#7c6aff", animation: `dotBounce 1.2s ease-in-out ${i * 0.2}s infinite`, boxShadow: "0 0 8px #7c6aff88" }} />
                ))}
              </div>
        ...
        {!loading && (
          <div style={{ marginTop: "20px", paddingTop: "16px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "flex-end" }}>
            <button onClick={onClose} style={{ padding: "8px 18px", borderRadius: "10px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)", color: "#94a3b8", fontSize: "13px", fontWeight: 500, cursor: "pointer" }}>
              Close
            </button>
          </div>
        )}
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "16px" }}>
        <div style={{ width: "40px", height: "40px", borderRadius: "12px", background: "rgba(124,106,255,0.15)", border: "1px solid rgba(124,106,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <svg width="18" height="18" viewBox="0 0 16 16" fill="none">
            <rect x="2" y="2" width="5" height="5" rx="1" fill="#7c6aff" />
            <rect x="9" y="2" width="5" height="5" rx="1" fill="#7c6aff" fillOpacity="0.5" />
            <rect x="2" y="9" width="5" height="5" rx="1" fill="#7c6aff" fillOpacity="0.5" />
            <rect x="9" y="9" width="5" height="5" rx="1" fill="#7c6aff" />
          </svg>
        </div>
  ...
  if (error || !data) return (
    <div style={{ minHeight: "100vh", background: "#080810", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ width: "60px", height: "60px", borderRadius: "16px", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M12 9v4M12 17h.01M5.07 19H19a2 2 0 001.75-2.96L13.73 4a2 2 0 00-3.46 0L4.25 16.04A2 2 0 005.07 19z" stroke="#ef4444" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </div>
        <p style={{ color: "#f87171", marginBottom: "8px", fontFamily: "DM Sans, sans-serif" }}>{error}</p>
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .portal-root::after { content: ''; position: fixed; bottom: -200px; right: -100px; width: 600px; height: 600px; background: radial-gradient(ellipse at center, rgba(56,189,248,0.05) 0%, transparent 70%); pointer-events: none; z-index: 0; }
        ...
        .stat-card { background: linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.01) 100%); border: 1px solid rgba(255,255,255,0.07); border-radius: 16px; padding: 16px 20px; backdrop-filter: blur(12px); }
      ...
      <div className="portal-root">
        <div className="portal-content">
          <header style={{
            ...
            backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 10, background: "rgba(8,8,16,0.8)",
          }}>
          ...
          <main style={{ maxWidth: "720px", margin: "0 auto", padding: "48px 24px 80px" }}>
            <div className="fade-up" style={{ marginBottom: "40px" }}>
              <h1 style={{ margin: "0 0 8px", fontSize: "clamp(24px, 4vw, 36px)", fontWeight: 700, letterSpacing: "-0.03em", color: "#f8fafc", lineHeight: 1.2 }}>
                {data.name}
              </h1>
              {data.description && (
                <p style={{ margin: "0 0 28px", fontSize: "15px", color: "#64748b", lineHeight: 1.6 }}>{data.description}</p>
              )}

              <div style={{ background: "linear-gradient(135deg, rgba(124,106,255,0.08) 0%, rgba(124,106,255,0.03) 100%)", border: "1px solid rgba(124,106,255,0.2)", borderRadius: "20px", padding: "24px", boxShadow: "0 0 40px rgba(124,106,255,0.08)" }}>
                ...
                <div style={{ height: "6px", background: "rgba(255,255,255,0.06)", borderRadius: "999px", overflow: "hidden", marginBottom: "16px" }}>
                  <div style={{
                    height: "100%", borderRadius: "999px", width: `${data.overall_progress}%`,
                    background: data.overall_progress === 100 ? "linear-gradient(90deg, #22c55e, #4ade80)" : "linear-gradient(90deg, #7c6aff, #a78bff)",
                    boxShadow: "0 0 12px rgba(124,106,255,0.5)", transition: "width 1.2s cubic-bezier(0.4,0,0.2,1)",
                  }} />
                </div>
            ...
            <div className="fade-up" style={{ animationDelay: "100ms", marginBottom: "36px", display: "flex", justifyContent: "center" }}>
              <button className="summary-btn" onClick={() => setShowSummary(true)} style={{
                display: "flex", alignItems: "center", gap: "10px",
                ...
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path d="M8 1.5l1.6 3.9L13.5 7l-3.9 1.6L8 12.5l-1.6-3.9L2.5 7l3.9-1.6L8 1.5z" stroke="#a78bff" strokeWidth="1.2" strokeLinejoin="round" fill="rgba(124,106,255,0.2)" />
                </svg>
      ...
      {showSummary && shareToken && (
        <SummaryModal shareToken={shareToken} onClose={() => setShowSummary(false)} />
      )}
    </>
  );
}
